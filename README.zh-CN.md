...
纯矢量减少错误，保留备份并重新执行完整 QA；其他错误仍须明确处理。
路径匹配同时比较形状、样式和重复次数，保留透明度 0，并保护原有文字位置及图形。

`preserve` 策略现在直接复制可确定的源参考文献区域，保留原字形、行距和双栏
布局，避免书目被再次排版、混译或相互覆盖。没有唯一标题的区域不会被猜测。
旧运行的书目映射不会静默重写；可用 `restore-reference-layout --run-dir runs/paper`
修复已生成候选，然后重新 `qa` 和逐页复核。跨栏说明续文的识别已修正。
源文字存在但未被引擎正确收集、图注的公式对象错位、扫描件/OCR 与复杂公式
仍需专门核验；本版本不宣称任意未知版式均可无损自动完成。
机器 QA 成功仍为 `qa_generated`，逐页视觉复核后才可 `accept`。
...
- 215 项单元测试不联网运行，覆盖内容合同、三种 Provider、ChatGPT 网页人工桥接、单片段批处理边界、Provider 评估、无人值守与受监督断点续跑、Codex CLI 版本漂移审计、受控源矢量重放与链尾回滚、双栏参考文献空间边界、自动误匹配显式排除、参考文献与透传映射、碎词安全审查、领域包身份、PDF 哈希绑定、图片/矢量对象门禁、受控单行/多行文字修复、页面 QA、隔离环境入口和演示产物；
